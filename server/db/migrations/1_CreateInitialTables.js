class CreateInitialTables extends require('../Migration') {
  ...
  async body(index, database) {
    ...
    // await sql.schema.dropTableIfExists('whitelisted_ips')

    if (!(await sql.schema.hasTable('whitelisted_ips'))) {
      await sql.schema.createTable('whitelisted_ips', t => {
        t.string('ip').notNullable().unique()
        t.timestamp('created_at').defaultTo(sql.fn.now())
        t.timestamp('updated_at')
      })
      done = true
      console.info('Table "whitelisted_ips" created.')
    }
