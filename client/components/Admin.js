...
class Admin extends Base {
  ...
  niceDate(d) {
    ...
    const a = d[0].split("-");
    const b = d[1].split(":");
    return (a[1] === "11" ? "Nov " : "Dec ") + a[2] + ", " + b[0] + ":" + b[1];
  }
  ...
  render() {
    ...
    return (
      <Container style={{ marginTop: 100 }}>
        {this.state.error ? (
          <p className="big error">{this.state.error}</p>
        ) : null}
        <div> </div>
        {this.showCodes()}
        <h2>Get a new code for</h2>
        <InputGroup className="mb-3" size={"lg"}>
          <InputGroup.Text id="basic-addon3">Discord user</InputGroup.Text>
          <FormControl
            id="basic-url"
          ...
          {this.state.errors.discordMember && (
            <div className="input-error">{this.state.errors.discordMember}</div>
          )}
        </InputGroup>
        <Button
          size={"lg"}
        ...
        <div>
          Load previous data about{" "}
          <Button variant="primary" onClick={() => this.getRedeemed(0)}>
            ARG#1
          </Button>{" "}
          <Button variant="primary" onClick={() => this.getRedeemed(1)}>
            ARG#2
          </Button>{" "}
          <Button variant="primary" onClick={() => this.getRedeemed(2)}>
            ARG#3
          </Button>{" "}
          <Button variant="primary" onClick={() => this.getRedeemed(3)}>
            ARG#4
          </Button>{" "}
          <Button variant="primary" onClick={() => this.getRedeemed(4)}>
            Community Events
          </Button>
        </div>
        {this.showResults()}
