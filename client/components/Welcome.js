...
class Welcome extends Base {
  ...
  async setCallback() {
    ...
    if (accessToken && discordUser) {
      ...
      if (res.success) {
        this.setStore({
          powerTimestamp: parseInt(parseInt(Date.now() / 10000).toString() + discordUser.discriminator)
        })
    ...
    if (needAuthentication === false) {
      ...
      return <Container style={{marginTop: 100}}>
        <div className={'noTokens m0Auto'}>
          <div className={'welcometo'}>Welcome to the<br/>Syn City NFT dApp</div>

          <Button size={'lg'} variant={'warning'} onClick={() => window.location = '/auth/discord/login'}>Connect
            your
            Discord</Button>
        </div>
      </Container>
